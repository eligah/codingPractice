...
virtual	double GetMolecularWeight(string compound)
	{
		toLower(compound);
...
public:// Constructor
		...
		: Compound(name)
	{
		...
		_boilingPoint = this->GetCriticalPoint(_chemical, "B");
		_meltingPoint = this->GetCriticalPoint(_chemical, "M");
		_molecularWeight = this->GetMolecularWeight(_chemical);
		_molecularFormula = this->GetMolecularStructure(_chemical);
	}
